...
public class DoubleProperty extends FastProperty {
    private static final Logger LOGGER = LoggerFactory.getLogger(DoubleProperty.class);
    private final double defaultValue;
    private final AtomicReference<Double> currentValue;  // a touch overkill, but keeps the pattern
    private static final ConcurrentHashMap<String, DoubleProperty> properties = new ConcurrentHashMap<String, DoubleProperty>();
     ...
     * @param defaultValue
     */
    private DoubleProperty(String propertyName, double defaultValue) {
        super(propertyName);
        this.defaultValue = defaultValue;
        this.currentValue = new AtomicReference<Double>(defaultValue);
    }
     ...
     * ...
     * DoubleProperty myProperty = DoubleProperty.findOrCreate("my.doubleproperty.name", 1.414213562373095);
     * ...
     * if(myProperty.get() < 2) {
     *     ...
     * }
     ...
     */
    public static DoubleProperty findOrCreate(String propertyName, double defaultValue) {
        DoubleProperty retval;
        if(properties.containsKey(propertyName)) {
            retval = properties.get(propertyName);
        }
        else {
            DoubleProperty newProperty = new DoubleProperty(propertyName, defaultValue);
