...
public class IntProperty extends FastProperty {
    ...
    private static final ConcurrentHashMap<String, IntProperty> properties = new ConcurrentHashMap<String, IntProperty>();
     ...
     * @param defaultValue
     */
    private IntProperty(String propertyName, int defaultValue) {
        super(propertyName);
        this.defaultValue = defaultValue;
        this.currentValue = new AtomicInteger(defaultValue);
    }
     ...
     * ...
     * IntProperty myProperty = IntProperty.findOrCreate("meaning.of.life", 42);
     * ...
     * if(myProperty.get() != 42) {
     *     ...
     * }
     * else {
     *     ...
     ...
     */
    public static IntProperty findOrCreate(String propertyName, int defaultValue) {
        IntProperty retval;
        if(properties.containsKey(propertyName)) {
            retval = properties.get(propertyName);
        }
        else {
            IntProperty newProperty = new IntProperty(propertyName, defaultValue);
